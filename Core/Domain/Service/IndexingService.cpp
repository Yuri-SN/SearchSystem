...
std::vector<std::string> IndexingService::filterWordsByLength(const std::vector<std::string>& words) {
...
std::vector<std::string> IndexingService::extractWords(const std::string& text) {
    ...
    while (stream >> word) {
        ...
        word.erase(std::remove_if(word.begin(), word.end(), [](unsigned char chr) { return std::ispunct(chr); }),
                   word.end());
